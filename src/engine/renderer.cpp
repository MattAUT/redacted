...
Renderer::InitialiseOpenGL(int screenWidth, int screenHeight)
{
    ...
    m_pWindow = SDL_CreateWindow("Dark", SDL_WINDOWPOS_UNDEFINED, 
...
Renderer::DrawSprite(Sprite& sprite, Vector2 position, float angle, DrawSpace drawSpace)
{
    ...
    Matrix4 m_pWorld;
    SetIdentity(m_pWorld);
    m_pWorld.m[0][0] = cosf(angle) * (sizeX);
    m_pWorld.m[0][1] = -sinf(angle) * (sizeX);
    m_pWorld.m[1][0] = sinf(angle) * (sizeY);
    m_pWorld.m[1][1] = cosf(angle) * (sizeY);
    m_pWorld.m[3][0] = drawX;
    m_pWorld.m[3][1] = drawY;

    m_pSpriteShader->SetMatrixUniform("uWorldTransform", m_pWorld);
...
Sprite*
Renderer::CreateStaticTextSprite(const char* text, TTF_Font* font, SDL_Color color, SpriteOrigin origin)
{
	Texture* pTexture = m_pTextureManager->GetTextTexture(text, font, color);
